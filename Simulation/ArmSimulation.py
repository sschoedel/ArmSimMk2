...
trail = np.empty([1,3])
# trail = np.append(trail, [[0, 3, 1]], axis=0)
prevEndPosition = np.array([0, 0, 0])  # setup for draw trail
drawAxesBool = True  # For use with toggle axes button
...
# The following three functions are different inverse kinematic solvers and only one is used at a time
